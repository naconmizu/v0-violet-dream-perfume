        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {products.map((product, index) => (
            <Link key={index} href={`/fragrancia/${product.slug}`}>
              <Card className="group overflow-hidden border-2 hover:border-accent transition-all duration-300 hover:shadow-xl cursor-pointer">
                <div className="relative aspect-[3/4] overflow-hidden bg-gradient-to-br from-primary/5 to-accent/5">
                  ...
                  />
                  <div className="absolute top-4 right-4 w-12 h-12 rounded-full bg-accent/90 flex items-center justify-center"></div>
                </div>
